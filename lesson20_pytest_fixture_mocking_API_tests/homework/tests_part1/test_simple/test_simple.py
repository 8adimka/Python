...
def test_sum():
    assert summer (3,2,16) == 21, "Функция не правильно складывает числа"

def test_big_number():
    assert summer (300,200) == "Большое число", "Функция не определяет большие числа"

def test_one_arg():
    assert summer(301) == "Мало аргументов", "Ошибка возникла при передаче более одного аргумента"

def test_wrong_arg():
    assert summer ('число Пи!') == "ОШИБКА", "Функция не перехватывает ошибки или делает это неверно"
