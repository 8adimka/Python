...
class DeepSeekSolver:
    def __init__(self):
        ...
        self.should_stop_typing = False

    def _wait_if_paused(self):
        while self.paused and not self.should_stop_typing:
            time.sleep(0.1)

    def toggle_pause(self) -> None:
        self.paused = not self.paused
        if not self.paused and self.processing_task:
            self.should_stop_typing = True
    ...
    def human_like_typing(self, text: str) -> None:
        ...
        try:
            ...
            for line in lines[1:]:
                ...
                if not self.prev_line_ended_with_colon:
                    while self.current_indent < line_indent and not self.should_stop_typing:
                        self._wait_if_paused()
                        if self.should_stop_typing:
                            break
...
def run_daemon():
    ...
    def on_press(key):
        try:
            if key == Key.f8:
                print("Process hotkey triggered")
                solver.process_task()
            elif key == Key.f9:
                print("Pause hotkey triggered")
                solver.toggle_pause()
