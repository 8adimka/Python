...
class CodePreprocessor:
    ...
    def clean_code(code: str) -> str:
        ...
        # Удаляем все лишние отступы
        lines = [line.strip() for line in code.split('\n') if line.strip()]
        
        # Простая логика добавления отступов после двоеточия
        cleaned_lines = []
        indent = 0
        
        for line in lines:
            # Удаляем все лишние пробелы в начале и конце строки
            clean_line = line.strip()
            
            # Добавляем отступ если нужно
            cleaned_lines.append(' ' * indent + clean_line)
            
            # Увеличиваем отступ после двоеточия
            if clean_line.endswith(':'):
                indent += 4
            # Сбрасываем отступ после return
...
class DeepSeekSolver:
    ...
    def send_to_api(self, prompt: str) -> Optional[str]:
        ...
        data = {
            ...
            "temperature": 0.2,  # Очень низкая температура для минимизации ошибок
            "max_tokens": 2000
        }
    ...
    def process_task(self) -> None:
        try:
            ...
            if solution:
                print(f"Получено решение: {solution}")
                clean_solution = solution.replace('```python', '').replace('```', '').strip()
                print(f"Ввод решения: {clean_solution}")
