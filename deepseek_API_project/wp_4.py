...
class DeepSeekSolver:
    ...
    def human_like_typing(self, text: str) -> None:
        ...
        try:
            ...
            if len(lines) < 2:  # Если меньше 2 строк, ничего не делаем
                return
                
            min_indent = min((len(line) - len(line.lstrip()) for line in lines[1:] if line.strip()), default=0)
            clean_lines = [lines[0]] + [line[min_indent:] for line in lines[1:]]
            ...
            for i, line in enumerate(clean_lines[1:], start=1):
                ...
                # Для первой печатаемой строки (второй в исходном тексте) пропускаем отступы
                if i > 1:  # Только для строк после второй
